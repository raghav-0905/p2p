...
function InvoiceForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      setToast({ open: true, msg: "Invoice created successfully!", severity: "success" });
      setPoId("");
      setGrnId("");
      setInvoiceNumber("");
      setInvoiceDate("");
      ...
      <Box display="flex" justifyContent="center" alignItems="center" minHeight="calc(100vh - 64px)" p={4}>
        ...
        >
          <Box display="flex" alignItems="center" gap={1.5} mb={1}>
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                Create an invoice linked to a PO and GRN for three-way matching.
              </Typography>
            </Box>
          </Box>
          ...
          <Box component="form" onSubmit={handleSubmit}>
            <TextField fullWidth label="Purchase Order ID (optional)" margin="normal" value={poId} onChange={(e) => setPoId(e.target.value)} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
            <TextField fullWidth label="GRN ID (optional)" margin="normal" value={grnId} onChange={(e) => setGrnId(e.target.value)} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
            <TextField fullWidth label="Invoice Number" margin="normal" required value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
            <TextField fullWidth type="date" label="Invoice Date" margin="normal" required InputLabelProps={{ shrink: true }} value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2 } }} />
